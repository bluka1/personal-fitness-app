...
/* ============================================================
   Sinkronizacija (Cloudflare Worker + KV). Lokalno je i dalje
   glavno — ovo je sloj iznad koji izjednačava uređaje. Auth =
   tajna fraza u headeru x-sync-secret; worker njome namespacea
   KV ključ, pa ista fraza = isti podaci. Worker: cloudflare-worker.js.
   ============================================================ */
...
function syncReady() { return !!(SYNC.cfg && SYNC.cfg.url && SYNC.cfg.secret); }

function loadSync() {
  SYNC.cfg = readLS(K_SYNC, null);
  SYNC.status = syncReady() ? "idle" : "off";
}
...
/* Jedan poziv prema workeru: GET vrati spremljeni blob (null ako prazan),
   PUT ga zapiše. Fraza ide u x-sync-secret. */
async function workerFetch(method, body) {
  const c = SYNC.cfg;
  const res = await fetch(c.url.replace(/\/+$/, "") + "/", {
    method: method,
    headers: { "content-type": "application/json", "x-sync-secret": c.secret },
    body: body ? JSON.stringify(body) : undefined,
  });
  if (res.status === 404) return null;
  if (!res.ok) throw new Error("HTTP " + res.status);
  return method === "GET" ? await res.json().catch(() => null) : null;
}
...
async function pushRemote() { await workerFetch("PUT", currentPayload()); }

async function pullRemote() { return await workerFetch("GET"); }
...
function sheetSync() {
  ...
  <div class="sheet-in">
    ...
    <p class="note">${tr("Podaci se sinkroniziraju preko tvog Cloudflare workera. Lokalno spremanje ostaje — aplikacija radi i bez interneta.")}</p>
    <label class="eyebrow">${tr("Worker URL")}</label>
    <input id="sy_url" value="${esc(c.url || "")}" placeholder="https://ime.tvoj.workers.dev" style="margin:6px 0 12px" autocomplete="off">
    <label class="eyebrow">${tr("Tajna fraza")}</label>
    <input id="sy_secret" type="password" value="${esc(c.secret || "")}" placeholder="${tr('barem 8 znakova')}" style="margin:6px 0 12px" autocomplete="off">
    <p class="note" style="margin:-4px 0 12px">${tr("Ista fraza na svim uređajima povezuje podatke. Čuvaj je — tko je zna, vidi tvoje podatke.")}</p>
    <button class="btn btn-p wide" data-act="sync-save">${tr("Spremi")}</button>
    <div id="sync_msg" class="note" style="margin-top:12px"></div>
    ${ready ? `
      <div class="row" style="gap:8px;margin:14px 0 10px">
      ...
      <button class="btn btn-g wide" style="margin-top:14px" data-act="sync-disable">${tr("Isključi sinkronizaciju")}</button>
    ` : ""}
    <p class="note" style="margin-top:14px;padding-top:12px;border-top:1px solid var(--ln)">
      ${tr("Postavljanje workera: vidi cloudflare-worker.js u repozitoriju. Deployaš ga jednom na svoj Cloudflare, upišeš URL i frazu ovdje.")}
    </p>
  </div>`;
}
...
document.addEventListener("click", (e) => {
  ...
  if (a === "sync-save") {
    ...
    const secret = $("#sy_secret").value.trim();
    if (!url || !secret) { syncMsg(tr("Upiši Worker URL i frazu.")); return; }
    if (secret.length < 8) { syncMsg(tr("Fraza mora imati barem 8 znakova.")); return; }
    SYNC.cfg = { url: url, secret: secret };
    saveSync();
    SYNC.status = "idle";
    render();
    syncMsg(tr("Spremljeno"));
    syncNow().then(() => syncMsg(syncStatusText()));
    return;
  }
  if (a === "sync-disable") {
    SYNC.cfg = null; saveSync(); SYNC.status = "off"; S.sheet = null; render();
    toast(tr("Sinkronizacija isključena"));
    return;
  }
